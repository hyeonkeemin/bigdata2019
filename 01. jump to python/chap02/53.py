input_data="201901300915-0175440.002"
year=input_data[0:4]
month=input_data[4:6]
day=input_data[6:8]
time=input_data[8:10]
minute=input_data[10:12]
temp=input_data[12:15]
dust=input_data[15:17]
u_dust=input_data[17:19]
ozon=input_data[19:]

print("year : ",year);print("month : ",month);print("day : ",day);print("time : ",time);print("minute : ",minute);print("temp : ",temp);print("dust : ",dust);print("u_dust : ",u_dust);print("ozon : ",ozon)
