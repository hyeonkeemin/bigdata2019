        ...
        if input_menu == '1':
            ...
            for student in root.iter('student'):
                ...
                if input_search in idc:
                    ...
                    if not student.iter('language'):
                        while True:
                            newinput_lang_name = input('사용가능한 언어를 입력하세요(종료는 "Enter") : ')
                            if newinput_lang_name == '':break
                            else:
                                newinput_lang_level = input('수준을 입력하세요 : ')
                                newinput_lang_period = input('사용기간을 입력하세요 : ')
                                prac = SubElement(student, )
                                language = SubElement(prac, 'practicable_computer_languages')
                                SubElement(prac, 'language', name=newinput_lang_name, level=newinput_lang_level)
                                SubElement(language,'period', value=newinput_lang_period)
                    else:
            ...
            for student in root.iter('student'):
                id_list = student.get('ID')
                if id_list == ID_del:
                    root.remove(student)
        elif input_menu == '6' :
            from xml.etree.ElementTree import ElementTree
            ElementTree(root).write('sample.xml',encoding='UTF-8',xml_declaration=True)
            break
